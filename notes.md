...
Constraining the set of potential senders of a message by paring public and private key.
* b.Secrecy: only the receiver can decrypt the message.
The private key (or “secret key”) must be zealously guarded, and can't be derived from public key.
...
Making sure that the sender uses the right public key, so the message won't be decrypted by man-in-the-middle.
