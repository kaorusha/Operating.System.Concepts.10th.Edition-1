...
#### 21.24
When a UT enters the kernel, its KT may block. In that case the kernel switches to a scheduling thread which is, by UMS terminology, called primary. It uses this thread to reenter the user-mode scheduler so that it can pick another UT to run. Eventually, a blocked KT will complete its operation and be ready to return to user mode.
#### 21.25
...
#### 21.26
The virtual address translation on most modern processors uses a multi-level page table.Thanks to this hierarchical mechanism, the total size of all page-table. Page-directory level is pre-defined according to the virtual address space supported by processor with a hierachical mechanism shown in figure 21.3 and figure 21.4. To improve performance, the MM maps the page-directory and PTE-table pages into the same contiguous region of virtual addresses in every process. Thus it does not require any additional virtual memory pages.
#### 21.27
